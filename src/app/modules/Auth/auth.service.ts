...
const mailSender = async (mailSender: any) => {
  const { name, email } = mailSender;
  ...
  try {
    await transporter.sendMail({
      ...
      to: email,
      subject: 'send a verification email',
      // text: website,
      date: new Date(),
      html: `<!doctype html>
    <html ⚡4email>
      ...
      <body>
      <div>  your verification code : 1234 </div>
      </body>
    </html>`,
    });
...
const sendCode = async (code: string) => {
  const userCode = parseInt(code?.code);
  const gearedCode = 1234;
  const verified = gearedCode === userCode;

  if (verified) {
    return 'Your mail is verified';
  } else {
    throw new AppError(httpStatus.UNAUTHORIZED, 'Please provide a valet code');
  }
};
